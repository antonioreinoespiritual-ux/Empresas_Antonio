...
export interface CompositionRendererProps {
  ...
  checkoutHref: string;
  /** Assets ya resueltos (Fase 5, §7) — ausente/vacío es válido, los nodos image/video/gallery caen a su placeholder honesto si no encuentran el assetId acá. */
  assets?: AssetMap;
}
 ...
 * una lista de `section` (garantía del propio schema de dominio). La
 * resolución de `assetId -> Asset` es responsabilidad exclusiva del caller
 * (apps/web) — este componente nunca hace I/O.
 */
export function CompositionRenderer({ composition, theme, brand, checkoutHref, assets = {} }: CompositionRendererProps) {
  return (
    <ThemeProvider theme={theme} brand={brand}>
      {composition.root.map((section) => (
        <SectionNode key={section.id} node={section} theme={theme} checkoutHref={checkoutHref} assets={assets} />
      ))}
    </ThemeProvider>
  );
}
